...
public class SorcererGeas extends GeasEffect {

    public SorcererGeas() {
        super(MalumGeasEffectTypeRegistry.PACT_OF_THE_SORCERER.get());
    }

    public static void increaseDetection(LivingEvent.LivingVisibilityEvent event) {
        if (GeasEffectHandler.hasGeasEffect(event.getEntity(), MalumGeasEffectTypeRegistry.PACT_OF_THE_SORCERER)) {
        ...
        if (target.getHealth() > target.getMaxHealth() * 0.95f) {
            ...
            WorldEventHandler.addWorldEvent(target.level(),
                    new DelayedDamageWorldEvent(target)
                            ...
                            .setImpactParticleEffect(ParticleEffectTypeRegistry.SHAKEN_FAITH, new MalumNetworkedParticleEffectColorData(SpiritTypeRegistry.WICKED_SPIRIT))
