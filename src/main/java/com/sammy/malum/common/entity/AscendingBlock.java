...
public class AscendingBlock extends Entity {
    ...
    static final EntityDataAccessor<BlockPos> DATA_START_POS =
            SynchedEntityData.defineId(AscendingBlock.class, EntityDataSerializers.BLOCK_POS);
...
//        this.maxRiseTicks = 200;
    ...
    private AscendingBlock(Level level, double x, double y, double z, BlockState state, float lifetime) {
        this(level);
    ...
    public static AscendingBlock rise(Level level, BlockPos pos, BlockState blockState, float lifetime) {
        AscendingBlock fallingblockentity = new AscendingBlock(level, (double) pos.getX() + (double) 0.5F, (double) pos.getY(), (double) pos.getZ() + (double) 0.5F, blockState.hasProperty(BlockStateProperties.WATERLOGGED) ? (BlockState) blockState.setValue(BlockStateProperties.WATERLOGGED, false) : blockState, lifetime);
