...
public class CardinalLobEntropyGoal extends Goal {
    ...
    public boolean canUse() {
        return cardinal.target != null && !cardinal.isWithinRetaliationBlastRadius();
    }

    @Override
    public boolean canContinueToUse() {
        return this.canUse() || !cardinal.getNavigation().isDone();
    }
            ...
            if (hasLineOfSight || isInLobbingState) {
                if (cardinal.entropyCharge == null) {
                    ...
                    if (attackTime == randomizedAttackInterval - LOB_ANIMATION_DELAY) {
                        cardinal.level().broadcastEntityEvent(cardinal, CardinalCultist.LOB_EVENT);
