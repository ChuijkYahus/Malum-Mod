...
public class HiddenBladeDelayedImpact extends ThrowableItemProjectile {
    ...
    protected void onHitEntity(EntityHitResult result) {
        if (getOwner() instanceof LivingEntity owner) {
            ...
            var source = DamageTypeHelper.create(level(), MalumDamageTypes.HIDDEN_BLADE_PHYSICAL_COUNTER, this, owner);
            ...
            if (success && target instanceof LivingEntity livingentity) {
                if (magicDamage > 0) {
                    if (!livingentity.isDeadOrDying()) {
                        ...
                        livingentity.hurt(DamageTypeHelper.create(level(), MalumDamageTypes.HIDDEN_BLADE_MAGIC_COUNTER, this, owner), magicDamage);
                    }
                }
                enemiesHit++;
            }
