...
public class CreepingBlightBlock extends Block {
    ...
    public CreepingBlightBlock(Properties p_49795_) {
        super(p_49795_);
        this.registerDefaultState(this.stateDefinition.any().setValue(HORIZONTAL_FACING, Direction.NORTH).setValue(BLIGHT_TYPE, BlightType.CLINGING_BLIGHT));
    }
    ...
    public boolean canSurvive(BlockState pState, LevelReader pLevel, BlockPos pPos) {
        ...
        if (value.equals(BlightType.CLINGING_BLIGHT) || value.equals(BlightType.SOULWOOD_ROOTS)) {
            ...
            if (value.equals(BlightType.CLINGING_BLIGHT)) {
                if (!Block.canSupportCenter(pLevel, pPos.relative(direction), direction.getOpposite())) {
                    return false;
                }
            }
        }
        if (value.equals(BlightType.HANGING_BLIGHT)) {
    ...
    public VoxelShape getShape(BlockState pState, BlockGetter pLevel, BlockPos pPos, CollisionContext pContext) {
        ...
        switch (pState.getValue(BLIGHT_TYPE)) {
            case SOULWOOD_ROOTS -> shapeArray = new VoxelShape[]{SHAPE_GROUNDED_ROOTS_X, SHAPE_GROUNDED_ROOTS_X, SHAPE_GROUNDED_ROOTS_Z, SHAPE_GROUNDED_ROOTS_Z};
            case CLINGING_BLIGHT -> shapeArray = new VoxelShape[]{SHAPE_ROOTED_EAST, SHAPE_ROOTED_WEST, SHAPE_ROOTED_SOUTH, SHAPE_ROOTED_NORTH};
            case SOULWOOD_SPIKE -> shapeArray = new VoxelShape[]{SHAPE_BRACED_EAST, SHAPE_BRACED_WEST, SHAPE_BRACED_SOUTH, SHAPE_BRACED_NORTH};
