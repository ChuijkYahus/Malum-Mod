...
public class QuartzSoundType extends ExtendedSoundType {
    ...
    public void onPlayBreakSound(Level level, BlockPos pos) {
        level.playLocalSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, MalumSoundEvents.QUARTZ_CLUSTER_BLOCK_BREAK.get(), SoundSource.BLOCKS, (getVolume() + 1.0F) / 6.0F, getPitch() * 1.8F, false);
    }

    @Override
    public void onPlayPlaceSound(Level level, BlockPos pos, Player player) {
        level.playSound(player, pos, MalumSoundEvents.QUARTZ_CLUSTER_BLOCK_PLACE.get(), SoundSource.BLOCKS, (getVolume() + 1.0F) / 8.0F, getPitch() * 1.6F);
    }

    @Override
    @OnlyIn(value = Dist.CLIENT)
    public void onPlayHitSound(BlockPos pos) {
        Minecraft.getInstance().getSoundManager().play(new SimpleSoundInstance(MalumSoundEvents.QUARTZ_CLUSTER_BLOCK_HIT.get(), SoundSource.BLOCKS, (getVolume() + 1.0F) / 12.0F, getPitch() * 1.65F, MalumMod.RANDOM, pos));
    }
}