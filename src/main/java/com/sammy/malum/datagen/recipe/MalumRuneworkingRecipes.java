...
public class MalumRuneworkingRecipes implements IConditionBuilder {
    ...
    protected static void buildRecipes(RecipeOutput recipeOutput) {
        var taintedRock = TAINTED_ROCK_SET.rock.block;
        var nullSlate = MalumContent.Materials.NULL_SLATE;

        var runewood = RUNEWOOD_SET.planks.block;
        var soulwood = SOULWOOD_SET.planks.block;

        new RunicWorkbenchRecipeBuilder(MalumContent.Gear.RUNE_OF_VITALITY, 1)
                .setPrimaryInput(taintedRock, 4)
