...
public class RitualPlinthParticleEffects {
    ...
    public static void failRitualParticles(RitualPlinthBlockEntity plinth, MalumNetworkedParticleEffectColorData colorData) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            var lightSpecs = spiritLightSpecs(level, ritualIconPos, spiritType, new WorldParticleOptions(i < 2 ? MalumParticles.STAR : LodestoneParticleTypes.TWINKLE_PARTICLE));
        ...
        for (int i = 0; i < 64; i++) {
            ...
            if (random.nextFloat() < 0.8f) {
                final WorldParticleOptions options = new WorldParticleOptions((star ? MalumParticles.STAR : MalumParticles.LIGHT_SPEC_SMALL));
                var lightSpecs = spiritLightSpecs(level, spawnPosition, spiritType, options);
                lightSpecs.getBuilder()
                ...
                lightSpecs.getBloomBuilder()
                        .multiplyLifetime(lifetimeMultiplier)
                        .addTickActor(slowDown)
                        .modifyData(AbstractParticleBuilder::getScaleData, d -> d.multiplyValue(star ? 2.4f : 1.2f))
                        .setMotion(motion);
                lightSpecs.spawnParticles();
            }
            if (!star && random.nextFloat() < 0.8f) {
                ...
                var sparks = SparkParticleEffects.spiritMotionSparks(level, spawnPosition, spiritType).act(b -> b.getParticleOptions().setBehavior(SparkParticleBehavior.sparkBehavior()));
                sparks.getBuilder()
    ...
    public static void incrementRitualTierParticles(RitualPlinthBlockEntity plinth, MalumNetworkedParticleEffectColorData colorData) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            final WorldParticleOptions options = new WorldParticleOptions(i < 2 ? MalumParticles.STAR : MalumParticles.LIGHT_SPEC_SMALL);
            var lightSpecs = spiritLightSpecs(level, ritualIconPos, spiritType, options);
            lightSpecs.getBuilder()
                    .setSpinData(SpinParticleData.create(0.4f, 0).setSpinOffset((level.getGameTime() * 0.05f) % 6.28f).setEasing(Easing.CUBIC_IN).build())
    ...
    public static void beginChargingParticles(RitualPlinthBlockEntity plinth, MalumNetworkedParticleEffectColorData colorData) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            var lightSpecs = spiritLightSpecs(level, plinthItemPos, spiritType, new WorldParticleOptions(i < 2 ? MalumParticles.STAR : LodestoneParticleTypes.TWINKLE_PARTICLE));
            lightSpecs.getBuilder()
        ...
        for (int i = 0; i < 128; i++) {
            int finalI = i;
            Vec3 particlePosition = VecHelper.rotatingRadialOffset(plinthItemPos, 0.75f, i, 64, gameTime, 160);
            Consumer<WorldParticleBuilder> behavior = b -> b.addTickActor(p -> {
                if (level.getGameTime() < gameTime + (finalI + 2)/2f) {
                    p.setParticleSpeed(0, 0.03f*(finalI/128f), 0);
                }
            });
            ...
            WorldParticleBuilder.create(MalumParticles.SPARK.get())
                    .setBehavior(DirectionalParticleBehavior.directional(plinthItemPos.subtract(particlePosition).normalize()))
                    ...
                    .setColorData(spiritType.createColorData().build().multiplyCoefficient(1.5f))
    ...
    public static void eatItemParticles(RitualPlinthBlockEntity plinth, Vec3 targetPos, MalumNetworkedParticleEffectColorData colorData, ItemStack stack) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            int finalI = i;
            Vec3 offsetPosition = VecHelper.rotatingRadialOffset(targetPos, 0.5f, i, 16, gameTime, 160);
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                WorldParticleBuilder.create(MalumParticles.SPARK)
                        .setBehavior(DirectionalParticleBehavior.directional(targetPos.subtract(particlePosition).normalize()))
    ...
    public static void holdingPrimeItemPlinthParticles(RitualPlinthBlockEntity plinth) {
        ...
        if (level.getGameTime() % 3L == 0) {
            for (int i = 0; i < 8; i++) {
                ...
                WorldParticleBuilder.create(MalumParticles.SPARK)
                        .setBehavior(DirectionalParticleBehavior.directional(itemPos.subtract(particlePosition).normalize()))
    ...
    public static void riteActivePlinthParticles(RitualPlinthBlockEntity plinth) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (gameTime % 6L == 0) {
                ...
                sparkParticles.getBuilder().setMotion(randomizedVelocity)
                        ...
                        .modifyData(AbstractParticleBuilder::getLengthData, d -> d.multiplyValue(1.75f).multiplyCoefficient(0.75f))
            ...
            if (gameTime % 12L == 0) {
                WorldParticleBuilder.create(gameTime % 24L == 0 ? MalumParticles.CIRCLE : MalumParticles.SQUARE)
                        .setBehavior(DirectionalParticleBehavior.directional(particleVelocity.normalize()))
                        ...
                        .setColorData(spiritType.createColorData().setCoefficient(0.75f).build())
                        .setLifetime(80)
                        .enableNoClip()
                        .spawn(level, particlePosition.x, particlePosition.y, particlePosition.z);
            }
            ...
            WorldParticleBuilder.create(MalumParticles.SPARK)
                    .setBehavior(DirectionalParticleBehavior.directional(particleDirection))
        ...
        if (gameTime % 16L == 0) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                WorldParticleBuilder.create(MalumParticles.RITUAL_CIRCLE)
                ...
                WorldParticleBuilder.create(MalumParticles.RITUAL_CIRCLE_WISP)
                        .setBehavior(DirectionalParticleBehavior.directional(new Vec3(0, 1, 0)))
