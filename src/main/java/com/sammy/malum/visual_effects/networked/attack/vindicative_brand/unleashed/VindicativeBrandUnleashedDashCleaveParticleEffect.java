...
public class VindicativeBrandUnleashedDashCleaveParticleEffect extends MalumNetworkedWeaponParticleEffectType<WeaponParticleEffectType.WeaponParticleEffectData> {

    public VindicativeBrandUnleashedDashCleaveParticleEffect(String id) {
        super(id);
    }
    ...
    public void act(Level level, RandomSource random, NetworkedParticleEffectPositionData positionData, MalumNetworkedParticleEffectColorData colorData, WeaponParticleEffectData extraData) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            slash.getBuilder()
                    .setMotion(slashDirection.scale(Easing.SINE_IN_OUT.asWeighedRandom(random, 0.7f, 0.9f)))
                    .setBehavior(PointyDirectionalParticleBehavior.pointyDirectional(slashDirection));
            for (int j = 0; j < 5; j++) {
                var spin = Easing.SINE_IN_OUT.asWeighedRandom(random, -0.05f, 0.05f);
                var scale = Easing.SINE_IN_OUT.asWeighedRandom(random, 4f, 5f) * (1 + j * 0.15f);
                var transparency = 1f - j * 0.2f;
                slash.getBuilder()
